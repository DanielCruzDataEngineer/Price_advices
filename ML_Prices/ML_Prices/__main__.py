...
def sendEmail(price, productUrl, preco_calculo, percent):
    ...
    body = """{} está a R${} no momento. \n\n Confira no link: {}\n\n Preço diminuiu R$ {} (Ou seja diminiu % {}) \n\n-Mercado Livre Price Bot""".format(
        title, price, productUrl, imagem, preco_calculo, percent
    )
    html = """\
    <html>
    ...
    """.format(
        title, price, productUrl, preco_calculo, percent, imagem
    )
...
itens = [
    {"url": mon_1, "price": 599.0, "email": False, "store": "mercado"},
...
if __name__ == "__main__":
    checkPrice(
        itens
    )  # Aqui é o tempo em que o programa irá rodar; no momento está em 1 hora (3600s)
